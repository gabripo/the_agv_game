...
|---------|---------|-----------------|-------------------|
| **Interior** | Wheel Odometry, IMU | θ + v measurement update | Always active (odometry: pre-integrated encoder bias; IMU: superimposed θ/v correction) |
...
- The **IMU** provides unbiased heading ($\theta$) and velocity ($v$) measurements via `imuUpdate()`, running **before** the wheel encoder update to correct the heading while the covariance is still large. At default accuracy, the IMU dominates the biased encoder and keeps the EKF on track inside the corridor.
- Only **wheel odometry** (pre-integrated biased encoder heading) + **IMU** ($\theta$/$v$ correction) remains
